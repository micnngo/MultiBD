...
/////////////////////////////////////////////////////////////
  ...
  scheme.for_each( boost::make_counting_iterator(0), boost::make_counting_iterator(kmax),
    [&](int w) {
      ...
      ig[w].resize(Ap1*Bp1);
      derivatives_lt_Cpp(s,lambda1,lambda2,alpha,beta,powI_inf,powI_rem,S0,I0,Ap1,Bp1,ord,direction,yvec,ig[w]);
    });

  mytype::ComplexNumber s(AA/(2*t),0.0);
  mytype::ComplexVector psum0(matsize);
  derivatives_lt_Cpp(s,lambda1,lambda2,alpha,beta,powI_inf,powI_rem,S0,I0,Ap1,Bp1,ord,direction,yvec,psum0);
        ...
        if (k > kmax) {
          ...
          scheme.for_each( boost::make_counting_iterator(0), boost::make_counting_iterator(nblocks),
            [&](int w) {
              ...
              derivatives_lt_Cpp(s,lambda1,lambda2,alpha,beta,powI_inf,powI_rem,S0,I0,Ap1,Bp1,ord,direction,yvec,ig[w+kmax]);
            });
